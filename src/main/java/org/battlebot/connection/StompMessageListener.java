...
public abstract class StompMessageListener implements Runnable {
	...
	@Override
	public void run() {
		LOGGER.debug("D�marrage �coute message Stomp ", cnx);
		while(!stop) {
			try {
				...
				//normal c'est pour pouvoir arreter l'�coute
			}catch (Exception e) {
				LOGGER.debug("R�ception message stomp :", e);
				stop = true;
			}
		}
		LOGGER.debug("Arret �coute message Stomp ", cnx);
